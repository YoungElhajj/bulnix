...
function inferCategory(productName: string): { name: string; slug: string; sortOrder: number } {
  ...
  // High-demand categories (sort order 0-9)
  if (name.includes("INSTAGRAM")) return { name: "Instagram", slug: "instagram", sortOrder: 0 };
  if (name.includes("FACEBOOK") || name.includes("FB")) return { name: "Facebook", slug: "facebook", sortOrder: 1 };
  if (name.includes("TIKTOK") || name.includes("TIK TOK")) return { name: "TikTok", slug: "tiktok", sortOrder: 2 };
  if (name.includes("DISCORD")) return { name: "Discord", slug: "discord", sortOrder: 3 };
  if (name.includes("TWITTER") || name.includes(" X ") || name.startsWith("X ") || name.includes("X OLD") || name.includes("X ACCOUNT")) return { name: "Twitter / X", slug: "twitter-x", sortOrder: 4 };
  if (name.includes("YOUTUBE")) return { name: "YouTube", slug: "youtube", sortOrder: 5 };
  if (name.includes("SNAPCHAT")) return { name: "Snapchat", slug: "snapchat", sortOrder: 6 };
  if (name.includes("REDDIT")) return { name: "Reddit", slug: "reddit", sortOrder: 7 };
  if (name.includes("ONLY FANS") || name.includes("ONLYFANS")) return { name: "OnlyFans", slug: "onlyfans", sortOrder: 8 };
  if (name.includes("LINKEDIN")) return { name: "LinkedIn", slug: "linkedin", sortOrder: 9 };

  // Streaming (sort order 10-19)
  if (name.includes("NETFLIX")) return { name: "Netflix", slug: "netflix", sortOrder: 10 };
  if (name.includes("SPOTIFY")) return { name: "Spotify", slug: "spotify", sortOrder: 11 };
  if (name.includes("APPLE MUSIC") || name.includes("APPLE TV")) return { name: "Apple", slug: "apple", sortOrder: 12 };
  if (name.includes("AMAZON") || name.includes("PRIME")) return { name: "Amazon", slug: "amazon", sortOrder: 13 };
  if (name.includes("DISNEY") || name.includes("HULU")) return { name: "Streaming", slug: "streaming", sortOrder: 14 };
  if (name.includes("STEAM")) return { name: "Gaming", slug: "gaming", sortOrder: 15 };

  // Software & Productivity (sort order 20-29)
  if (name.includes("CANVA")) return { name: "Design Tools", slug: "design-tools", sortOrder: 20 };
  if (name.includes("CHATGPT") || name.includes("OPENAI") || name.includes("DEEP SEEK") || name.includes("DEEPSEEK")) return { name: "AI Tools", slug: "ai-tools", sortOrder: 21 };
  if (name.includes("GMAIL") || name.includes("GOOGLE VOICE")) return { name: "Google", slug: "google", sortOrder: 22 };
  if (name.includes("MICROSOFT") || name.includes("OUTLOOK") || name.includes("OFFICE")) return { name: "Microsoft", slug: "microsoft", sortOrder: 23 };

  // VPN & Proxy (sort order 30-39)
  if (name.includes("VPN") || name.includes("NORDVPN") || name.includes("EXPRESSVPN") || name.includes("HMA") || name.includes("IP VANISH") || name.includes("IPVANISH")) return { name: "VPN", slug: "vpn", sortOrder: 30 };
  if (name.includes("PROXY") || name.includes("SOCKS") || name.includes("SOCK 5") || name.includes("9PROXY") || name.includes("PIA SOCK")) return { name: "Proxies", slug: "proxies", sortOrder: 31 };

  // Dating & Adult (sort order 40-49)
  if (name.includes("DATING") || name.includes("TINDER") || name.includes("BUMBLE")) return { name: "Dating", slug: "dating", sortOrder: 40 };

  // Communication (sort order 50-59)
  if (name.includes("TALKATONE") || name.includes("TEXT FREE") || name.includes("GOOGLE VOICE")) return { name: "Phone & SMS", slug: "phone-sms", sortOrder: 50 };
  if (name.includes("WHATSAPP") || name.includes("TELEGRAM")) return { name: "Messaging", slug: "messaging", sortOrder: 51 };

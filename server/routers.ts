...
export const appRouter = router({
  ...
  admin: router({
    ...
    orders: router({
      list: adminProcedure
        .input(z.object({ page: z.number().default(1), limit: z.number().default(50), status: z.string().optional(), search: z.string().optional() }))
        .query(({ input }) => db.adminGetOrders(input)),
      getDetail: adminProcedure
        .input(z.object({ orderId: z.number() }))
        .query(({ input }) => db.adminGetOrderDetail(input.orderId)),
      update: adminProcedure

    ...
    <div className="min-h-screen bg-[#061A2B] text-white"><Navbar />
      <div className="container pt-28 pb-16 animate-pulse">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="aspect-square bg-[#0A2540] rounded-2xl" />
          <div className="space-y-4"><div className="h-8 bg-[#0A2540] rounded w-3/4"/><div className="h-6 bg-[#0A2540] rounded w-1/4"/><div className="h-32 bg-[#0A2540] rounded"/></div>
        </div>
      </div>
    </div>
  );
  ...
  if (!product) return (
    <div className="min-h-screen bg-[#061A2B] text-white"><Navbar />
      <div className="container pt-28 pb-16 text-center">
        ...
        <Link href="/products"><Button className="bg-[#00C2FF] hover:bg-[#00a8d4] text-white mt-4">Browse Products</Button></Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#061A2B] text-white"><Navbar />
      <div className="pt-24 pb-8 border-b border-white/5">
        <div className="container">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <Link href="/" className="hover:text-[#00C2FF]">Home</Link><span>/</span>
            <Link href="/products" className="hover:text-[#00C2FF]">Products</Link><span>/</span>
      ...
      <div className="container py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="aspect-square bg-gradient-to-br from-[#0A2540] to-[#1e293b] rounded-2xl flex items-center justify-center overflow-hidden border border-white/5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              {p.isFeatured && <Badge className="bg-[#00C2FF] text-white border-0 text-xs">Featured</Badge>}
              <Badge className={inStock ? "bg-[#00C2FF]/10 text-[#00C2FF] border-[#00C2FF]/20" : "bg-red-500/10 text-red-400 border-red-500/20"}>
            ...
            <div className="text-4xl font-bold text-[#00C2FF] mb-6">${Number(p.customerPriceUSD).toFixed(2)} <span className="text-base text-slate-500 font-normal">USD</span></div>
            ...
            <div className="flex gap-3 mb-8">
              <Button className="flex-1 h-12 bg-[#00C2FF] hover:bg-[#00a8d4] text-white font-semibold" onClick={handleAdd} disabled={!inStock} style={{boxShadow:"0 0 20px rgba(0,194,255,0.3)"}}>
                <ShoppingCart className="h-5 w-5 mr-2"/> Add to Cart
              </Button>
              <Link href="/cart" className="flex-1">
                <Button variant="outline" className="w-full h-12 border-[#0F3D5E] text-slate-300 hover:text-white hover:bg-[#0F3D5E]/30" onClick={handleAdd} disabled={!inStock}>
                  Buy Now
                </Button>
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[{icon:Zap,text:"Instant Delivery"},{icon:Shield,text:"Secure Payment"},{icon:Package,text:"Digital Product"},{icon:Zap,text:"24/7 Support"}].map((f,i)=>(
                <div key={i} className="flex items-center gap-2 text-sm text-slate-400"><f.icon className="h-4 w-4 text-[#00C2FF]"/>{f.text}</div>
              ))}
            </div>
            {p.deliveryNote && <div className="mt-6 p-4 glass-card rounded-xl border-l-4 border-[#00C2FF]"><p className="text-sm text-slate-300">{p.deliveryNote}</p></div>}
          </div>
        </div>
      </div>
      <Footer/>

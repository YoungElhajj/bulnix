...
export default function OrderDetail() {
  ...
  if (!order) return (
    <div className="min-h-screen bg-[#061A2B] text-white flex items-center justify-center">
      <div className="text-center"><Package className="h-12 w-12 text-slate-700 mx-auto mb-4"/><h2 className="text-xl font-bold mb-2">Order not found</h2><Link href="/orders"><Button className="bg-[#00C2FF] hover:bg-[#00a8d4] text-white mt-3">Back to Orders</Button></Link></div>
    </div>
  );

  const o = order as any;
  const deliveries = (delivery as any[]) ?? [];
  const statusBadge = { pending: "bg-yellow-500/10 text-yellow-400", processing: "bg-blue-500/10 text-blue-400", completed: "bg-[#00C2FF]/10 text-[#00C2FF]", failed: "bg-red-500/10 text-red-400" }[o.status as string] ?? "bg-slate-500/10 text-slate-400";
      ...
      <div className="container py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            {deliveries.length > 0 && (
              <div className="glass-card rounded-xl p-6">
                <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><CheckCircle className="h-5 w-5 text-[#00C2FF]"/> Delivery Details</h2>
                <div className="space-y-3">
                  {deliveries.map((d: any, i: number) => (
                    <div key={i} className="p-4 bg-[#00C2FF]/5 border border-[#00C2FF]/20 rounded-xl">
          ...
          <div className="glass-card rounded-xl p-6 h-fit">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="border-t border-[#0F3D5E] pt-3 flex justify-between font-bold"><span className="text-white">Total</span><span className="text-[#00C2FF]">${Number(o.totalUSD).toFixed(2)}</span></div>
            </div>
            <Link href="/tickets"><Button variant="outline" className="w-full mt-5 border-[#0F3D5E] text-slate-300 hover:text-white hover:bg-[#0F3D5E]/30 text-sm">Need Help? Open Ticket</Button></Link>
          </div>
        </div>
      </div>
      <Footer/>

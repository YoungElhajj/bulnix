  ...
  return (
    <div className="min-h-screen bg-[#061A2B] text-white"><Navbar/>
      <div className="pt-24 pb-8 border-b border-white/5">
        <div className="container flex items-center justify-between flex-wrap gap-4">
          ...
          <Select value={status} onValueChange={v => { setStatus(v); setPage(1); }}>
            <SelectTrigger className="w-[160px] bg-[#0A2540] border-[#0F3D5E] text-white h-9"><SelectValue/></SelectTrigger>
      ...
      <div className="container py-8">
        ...
        : orders.length === 0 ? (
          <div className="text-center py-20"><Package className="h-16 w-16 text-slate-700 mx-auto mb-4"/><h3 className="text-xl font-semibold text-white mb-2">No orders found</h3><Link href="/products"><Button className="mt-4 bg-[#00C2FF] hover:bg-[#00a8d4] text-white">Browse Products</Button></Link></div>
        ) : (
          <>
                ...
                <Link key={order.id} href={"/orders/" + order.id}>
                  <div className="glass-card rounded-xl p-5 cursor-pointer hover:border-white/20 transition-all group">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-lg bg-[#00C2FF]/10 flex items-center justify-center flex-shrink-0"><Package className="h-5 w-5 text-[#00C2FF]"/></div>
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <ChevronRight className="h-4 w-4 text-slate-600 group-hover:text-[#00C2FF] transition-colors ml-auto mt-1"/>
                      </div>
                    </div>
                  </div>
                </Link>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-6">
                <Button variant="outline" className="border-[#0F3D5E] text-slate-400 hover:text-white hover:bg-[#0F3D5E]/30" disabled={page===1} onClick={()=>setPage(p=>p-1)}>Previous</Button>
                <span className="text-slate-500 text-sm px-4">Page {page} of {totalPages}</span>
                <Button variant="outline" className="border-[#0F3D5E] text-slate-400 hover:text-white hover:bg-[#0F3D5E]/30" disabled={page===totalPages} onClick={()=>setPage(p=>p+1)}>Next</Button>
              </div>
            )}
          </>
        )}

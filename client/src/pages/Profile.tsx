    ...
    <div className="min-h-screen bg-[#0B0F19] text-white"><Navbar/>
      ...
      <div className="container py-8 max-w-2xl mx-auto">
        <div className="glass-card rounded-xl p-6 mb-6 flex items-center gap-5">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#00B9E9] to-[#22C55E] flex items-center justify-center text-2xl font-bold text-white flex-shrink-0">
        ...
        <div className="glass-card rounded-xl p-6 space-y-5">
          ...
          <div><Label className="text-slate-300 text-sm mb-1.5 block">Display Name</Label><Input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Your name" className="bg-[#0F172A] border-white/10 text-white placeholder:text-slate-600 focus:border-[#00B9E9] h-10"/></div>
          <div><Label className="text-slate-300 text-sm mb-1.5 block">Email</Label><Input value={user?.email ?? ""} disabled className="bg-[#0F172A] border-white/10 text-slate-500 h-10 cursor-not-allowed"/></div>
          <div><Label className="text-slate-300 text-sm mb-1.5 block">Country</Label><Input value={form.country} onChange={e=>setForm(f=>({...f,country:e.target.value}))} placeholder="e.g. Nigeria" className="bg-[#0F172A] border-white/10 text-white placeholder:text-slate-600 focus:border-[#00B9E9] h-10"/></div>
          <div><Label className="text-slate-300 text-sm mb-1.5 block">Preferred Currency</Label>
            <Select value={form.preferredCurrency} onValueChange={(v:any)=>setForm(f=>({...f,preferredCurrency:v}))}>
              <SelectTrigger className="bg-[#0F172A] border-white/10 text-white h-10"><SelectValue/></SelectTrigger>
              <SelectContent className="bg-[#0F172A] border-white/10"><SelectItem value="USD">USD</SelectItem><SelectItem value="NGN">NGN</SelectItem><SelectItem value="EUR">EUR</SelectItem><SelectItem value="GBP">GBP</SelectItem></SelectContent>
            </Select>
          </div>
          ...
          <Button className="w-full h-11 bg-[#00B9E9] hover:bg-[#00a8d4] text-white font-semibold" onClick={()=>updateProfile.mutate(form)} disabled={updateProfile.isPending}>

  ...
  return (
    <div className="min-h-screen bg-[#061A2B] text-white"><Navbar/>
      ...
      <div className="container py-8">
        ...
        : products.length === 0 ? (
          <div className="text-center py-20"><Heart className="h-16 w-16 text-slate-700 mx-auto mb-4"/><h3 className="text-xl font-semibold text-white mb-2">No saved items</h3><Link href="/products"><Button className="bg-[#00C2FF] hover:bg-[#00a8d4] text-white mt-4">Browse Products</Button></Link></div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {products.map((p: any) => (
              <div key={p.id} className="product-card group">
                ...
                <div className="p-4">
                  <Link href={"/products/" + p.slug}><h3 className="text-sm font-semibold text-white line-clamp-2 mb-3 hover:text-[#00C2FF] transition-colors">{p.title}</h3></Link>
                  <div className="flex items-center justify-between mb-3"><span className="text-[#00C2FF] font-bold">${Number(p.customerPriceUSD).toFixed(2)}</span></div>
                  <Button size="sm" className="w-full bg-[#00C2FF]/10 hover:bg-[#00C2FF] text-[#00C2FF] hover:text-white border border-[#00C2FF]/20 hover:border-[#00C2FF] transition-all text-xs"

      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              { icon: Users, label: "Total Users", value: s?.totalUsers ?? 0, color: "#00B9E9", link: "/admin/users" },
              { icon: Package, label: "Products", value: s?.totalProducts ?? 0, color: "#22C55E", link: "/admin/products" },
              ...
              { icon: Activity, label: "Pending Orders", value: s?.pendingOrders ?? 0, color: "#00B9E9", link: "/admin/orders" },
              { icon: Package, label: "Visible Products", value: s?.visibleProducts ?? 0, color: "#22C55E", link: "/admin/products" },
            ].map((stat, i) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass-card rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Sync Products", href: "/admin/providers", color: "#00B9E9" },
                  { label: "Manage Products", href: "/admin/products", color: "#22C55E" },
                  { label: "View Orders", href: "/admin/orders", color: "#8B5CF6" },
                  { label: "Reply Tickets", href: "/admin/tickets", color: "#F59E0B" },
                ].map((a, i) => (
                  <Link key={i} href={a.href}>
                    <div className="p-3 rounded-lg border border-white/10 hover:border-white/20 cursor-pointer transition-all text-sm font-medium text-center" style={{color:a.color}}>{a.label}</div>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between">
                    ...
                    <Badge className={item.status === "active" ? "bg-[#22C55E]/10 text-[#22C55E] border-0 text-xs" : "bg-yellow-500/10 text-yellow-400 border-0 text-xs"}>{item.status}</Badge>
                  </div>
                ))}
              </div>
            </div>

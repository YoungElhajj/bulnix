  ...
  return (
    <AdminLayout title="Admin Dashboard">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#0D2137]">Overview</h1>
          <p className="text-[#4A6080] text-sm mt-0.5">Real-time platform metrics</p>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">{[...Array(8)].map((_,i)=><div key={i} className="bg-white border border-[#D8E8F5] shadow-sm rounded-xl p-5 h-24 animate-pulse"/>)}</div>
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <Link key={i} href={stat.link}>
                <div className="bg-white border border-[#D8E8F5] shadow-sm rounded-xl p-5 cursor-pointer hover:border-[#D8E8F5] transition-all">
                  <div className="flex items-center justify-between mb-2">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{background:stat.color+"15"}}>
                      <stat.icon className="h-4 w-4" style={{color:stat.color}}/>
                    </div>
                  </div>
                  <div className="text-xl font-bold text-[#0D2137]">{stat.value}</div>
                  <div className="text-xs text-[#4A6080] mt-0.5">{stat.label}</div>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white border border-[#D8E8F5] shadow-sm rounded-xl p-6">
              <h3 className="text-base font-bold text-[#0D2137] mb-4">Quick Actions</h3>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <Link key={i} href={a.href}>
                    <div className="p-3 rounded-lg border border-[#D8E8F5] hover:border-[#D8E8F5] cursor-pointer transition-all text-sm font-medium text-center" style={{color:a.color}}>{a.label}</div>
                  </Link>
                ))}
              </div>
            </div>
            <div className="bg-white border border-[#D8E8F5] shadow-sm rounded-xl p-6">
              <h3 className="text-base font-bold text-[#0D2137] mb-4">System Status</h3>
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between">
                    <span className="text-sm text-[#4A6080]">{item.label}</span>
                    <Badge className={item.status === "active" ? "bg-[#EEF4FF] text-[#0050D0] border-0 text-xs" : "bg-yellow-500/10 text-yellow-400 border-0 text-xs"}>{item.status}</Badge>
                  </div>
                ))}
              </div>
            </div>

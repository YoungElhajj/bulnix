      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-64">
            ...
            <Input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search products..." className="pl-9 bg-[#0d1117] border-emerald-900/40 text-white placeholder:text-slate-400 focus:border-emerald-500 h-9" />
          </div>
          <Button className="bg-emerald-600 hover:bg-emerald-500 text-white h-9" onClick={() => setAddOpen(true)}>
        ...
        <div className="bg-[#161b22] border border-emerald-900/30 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {products.map((p: any) => (
                  <tr key={p.id} className="border-b border-emerald-900/30 hover:bg-white/5 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-lg bg-[#0d1117] flex items-center justify-center flex-shrink-0 overflow-hidden">
                          {p.imageUrl
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-slate-400 truncate max-w-[200px]">{p.slug}</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-4 hidden md:table-cell">
                      <Badge className="bg-emerald-500/10 text-emerald-400 border-0 text-xs">{p.providerKey ?? "manual"}</Badge>
                    </td>
                    <td className="p-4 text-right text-slate-400">${Number(p.supplierPrice ?? 0).toFixed(2)}</td>
                    <td className="p-4 text-right text-slate-400">{p.markupPercent ?? 20}%</td>
                    <td className="p-4 text-right font-semibold text-emerald-400">${Number(p.customerPriceUSD ?? 0).toFixed(2)}</td>
                    <td className="p-4 text-center text-slate-400">{p.stockUnlimited ? "∞" : p.stockQuantity}</td>
                    <td className="p-4 text-center">
                      <button onClick={() => updateProduct.mutate({ id: p.id, isVisible: !p.isVisible })} className={"w-8 h-8 rounded-full flex items-center justify-center mx-auto transition-colors " + (p.isVisible ? "bg-emerald-500/10 text-emerald-400 hover:bg-[#00C2FF]/20" : "bg-slate-500/10 text-slate-400 hover:bg-slate-500/20")}>
                        {p.isVisible ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                      </button>
                    </td>
                    <td className="p-4 text-center">
                      <button onClick={() => updateProduct.mutate({ id: p.id, isFeatured: !p.isFeatured })} className={"w-8 h-8 rounded-full flex items-center justify-center mx-auto transition-colors " + (p.isFeatured ? "bg-yellow-500/10 text-yellow-400 hover:bg-yellow-500/20" : "bg-slate-500/10 text-slate-400 hover:bg-slate-500/20")}>
                        <Star className={"h-4 w-4 " + (p.isFeatured ? "fill-yellow-400" : "")} />
                      </button>
                    </td>
                    <td className="p-4 text-center">
                      <button onClick={() => openEdit(p)} className="w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-[#00C2FF]/20 flex items-center justify-center mx-auto transition-colors">
          ...
          {total > 50 && (
            <div className="flex items-center justify-center gap-2 p-4 border-t border-emerald-900/30">
              ...
              <Button variant="outline" className="border-emerald-900/30 text-slate-400 hover:text-white hover:bg-[#0d1117] h-8 text-xs" disabled={products.length < 50} onClick={() => setPage(p => p + 1)}>Next</Button>
            </div>
          )}
        </div>
      )}
        ...
        <DialogContent className="bg-[#161b22] border-emerald-900/30 text-white max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          {editing && (
            <div className="space-y-4 mt-2">
              <div>
                <Label className="text-slate-400 text-sm mb-1.5 block">Title Override</Label>
                <Input value={editForm.title} onChange={e => setEditForm((f: any) => ({ ...f, title: e.target.value }))} className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
              </div>
              <div>
                <Label className="text-slate-400 text-sm mb-1.5 block">Description Override</Label>
                <Textarea value={editForm.description} onChange={e => setEditForm((f: any) => ({ ...f, description: e.target.value }))} rows={3} className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 resize-none" />
              </div>
              <div>
                <Label className="text-slate-400 text-sm mb-1.5 block">Product Icon / Logo</Label>
                <div className="flex gap-2 items-start">
                  <div className="flex-1 space-y-2">
                    <Input value={editForm.imageUrl} onChange={e => setEditForm((f: any) => ({ ...f, imageUrl: e.target.value }))} placeholder="Paste URL or upload file below" className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
                    <label className="flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed border-emerald-900/30 hover:border-[#0050D0]/50 cursor-pointer transition-colors bg-[#0d1117] text-xs text-slate-400 hover:text-emerald-400">
                  ...
                  <div className="w-16 h-16 rounded-xl bg-[#0d1117] border border-emerald-900/30 flex items-center justify-center overflow-hidden flex-shrink-0">
                    {editForm.imageUrl
              ...
              <div>
                ...
                <Select value={editForm.categoryId || "none"} onValueChange={v => setEditForm((f: any) => ({ ...f, categoryId: v === "none" ? "" : v }))}>
                  <SelectTrigger className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10">
                    <SelectValue placeholder="Select category" />
                  </SelectTrigger>
                  <SelectContent className="bg-[#161b22] border-emerald-900/30 text-white max-h-48">
              ...
              <div>
                <Label className="text-slate-400 text-sm mb-1.5 block">Markup % (applied to supplier price)</Label>
                <Input type="number" value={editForm.markupPercent} onChange={e => setEditForm((f: any) => ({ ...f, markupPercent: Number(e.target.value) }))} className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
              </div>
              <div>
              ...
              <Button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white" onClick={handleUpdate} disabled={updateProduct.isPending}>
        ...
        <DialogContent className="bg-[#161b22] border-emerald-900/30 text-white max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <Input value={addForm.title} onChange={e => setAddForm(f => ({ ...f, title: e.target.value, slug: e.target.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").slice(0, 80) }))} placeholder="Product title" className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
            </div>
            <div>
              <Label className="text-slate-400 text-sm mb-1.5 block">Slug *</Label>
              <Input value={addForm.slug} onChange={e => setAddForm(f => ({ ...f, slug: e.target.value }))} placeholder="product-slug" className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
            </div>
            <div>
              <Label className="text-slate-400 text-sm mb-1.5 block">Description</Label>
              <Textarea value={addForm.description} onChange={e => setAddForm(f => ({ ...f, description: e.target.value }))} rows={3} placeholder="Product description..." className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 resize-none" />
            </div>
            <div>
              <Label className="text-slate-400 text-sm mb-1.5 block">Product Icon / Logo</Label>
              <div className="flex gap-2 items-start">
                <div className="flex-1 space-y-2">
                  <Input value={addForm.imageUrl} onChange={e => setAddForm(f => ({ ...f, imageUrl: e.target.value }))} placeholder="Paste URL or upload file below" className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
                  <label className="flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed border-emerald-900/30 hover:border-[#0050D0]/50 cursor-pointer transition-colors bg-[#0d1117] text-xs text-slate-400 hover:text-emerald-400">
                    {uploading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}
                    {uploading ? "Uploading..." : "Upload image file (PNG, JPG, SVG, max 5MB)"}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-slate-400 text-sm mb-1.5 block">Supplier Price (USD) *</Label>
                <Input type="number" step="0.01" value={addForm.supplierPrice} onChange={e => setAddForm(f => ({ ...f, supplierPrice: e.target.value }))} placeholder="0.00" className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
              </div>
              <div>
                <Label className="text-slate-400 text-sm mb-1.5 block">Markup %</Label>
                <Input type="number" value={addForm.markupPercent} onChange={e => setAddForm(f => ({ ...f, markupPercent: e.target.value }))} className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10" />
              </div>
            </div>
            {addForm.supplierPrice && (
              <p className="text-xs text-emerald-400">Customer price: ${(Number(addForm.supplierPrice) * (1 + Number(addForm.markupPercent) / 100)).toFixed(2)}</p>
            )}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-slate-400 text-sm mb-1.5 block">Stock Quantity</Label>
                <Input type="number" value={addForm.stockQuantity} onChange={e => setAddForm(f => ({ ...f, stockQuantity: e.target.value }))} disabled={addForm.stockUnlimited} className="bg-[#0d1117] border-emerald-900/30 text-white focus:border-emerald-500 h-10 disabled:opacity-50" />
              </div>
              <div className="flex items-end pb-1">
                <div className="flex items-center justify-between w-full">
                  <Label className="text-slate-400 text-sm">Unlimited Stock</Label>
            ...
            <Button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white" onClick={handleAdd} disabled={addProduct.isPending}>

...
export default function AdminLogs() {
  ...
  return (
    <AdminLayout title="System Logs">
      ...
      {isLoading ? (
        <div className="space-y-2">{[...Array(10)].map((_,i)=><div key={i} className="glass-card rounded-xl h-12 animate-pulse"/>)}</div>
      ) : logs.length === 0 ? (
        <div className="glass-card rounded-xl p-12 text-center">
          ...
          <p className="text-slate-500 text-sm">System activity logs will appear here.</p>
        </div>
      ) : (
              ...
              <tbody>
                {logs.map((log: any, i: number) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    ...
                    <td className="p-3 text-slate-300 max-w-[400px] truncate">{log.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>

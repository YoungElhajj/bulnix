...
async function confirmWalletTopup(reference, skipVerify = false, overrideAmountUSD) {
  ...
  try {
    ...
    if (wasFirstDeposit) {
      const [depositor] = await db.select({ referredBy: users.referredBy }).from(users).where(eq3(users.id, txn.userId)).limit(1);
      if (depositor?.referredBy) {
        const [referrer] = await db.select({ id: users.id }).from(users).where(eq3(users.referralCode, depositor.referredBy)).limit(1);
        if (referrer) {
          const bonus = 0.5;
          await db.insert(affiliateBalances).values({ userId: referrer.id, balanceUSD: bonus.toFixed(6), totalEarned: bonus.toFixed(6) }).onDuplicateKeyUpdate({ set: { balanceUSD: sql`balanceUSD + ${bonus.toFixed(6)}`, totalEarned: sql`totalEarned + ${bonus.toFixed(6)}` } });
...
var TABLE_MIGRATIONS, COLUMN_MIGRATIONS, SEED_MIGRATIONS;
